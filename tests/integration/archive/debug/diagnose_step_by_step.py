"""단계별 상세 비교 - PyHYSPLIT vs HYSPLIT Web"""
import sys
...
print("="*90)
print("  단계별 상세 비교 - PyHYSPLIT vs HYSPLIT Web")
print("="*90)
...
for i, hysplit in enumerate(hysplit_results):
    if i < len(trajectory):
        ...
        age = pyhysplit[0] / 3600.0  # seconds to hours
...
print(f"  표준편차: {np.std(cumulative_h_errors):.2f} km")
...
print(f"  표준편차: {np.std(cumulative_p_errors):.1f} hPa")

# 오차 증가율 분석
print(f"\n오차 증가 패턴:")
for i in range(1, len(cumulative_h_errors)):
    h_increase = cumulative_h_errors[i] - cumulative_h_errors[i-1]
    p_increase = cumulative_p_errors[i] - cumulative_p_errors[i-1]
    print(f"  Hour {i-1} → {i}: H_err +{h_increase:6.2f} km, P_err +{p_increase:6.1f} hPa")

# 초기 오차 분석
print(f"\n초기 오차 (0시간):")
print(f"  수평: {cumulative_h_errors[0]:.2f} km")
print(f"  압력: {cumulative_p_errors[0]:.1f} hPa")
print(f"\n이것은 시작 위치 변환 차이를 나타냅니다.")
print(f"PyHYSPLIT 시작 압력: {trajectory[0][3]:.1f} hPa")
print(f"HYSPLIT Web 시작 압력: {hysplit_results[0]['pressure']:.1f} hPa")
print(f"차이: {trajectory[0][3] - hysplit_results[0]['pressure']:.1f} hPa")
